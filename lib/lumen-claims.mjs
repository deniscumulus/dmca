...
export async function runLumenClaimsQueueScan(options = {}) {
  ...
  const existingClaimIndex = indexExistingClaims(existingClaims);
    ...
    for (const [domainIndex, domain] of domains.entries()) {
      ...
      try {
        ...
        for (const [requestIndex, row] of requestRows.entries()) {
          ...
          const existingNotice = options.forceRefreshKnown
            ? null
            : findExistingNotice(existingClaimIndex, domain, row);
          const notice = existingNotice
            ? hydrateExistingNotice(domain, row, existingNotice)
            : normalizeNotice(domain, row, await extractRequestDetails(page, row.requestId, domain));
  ...
  const run = {
    ...
    cachedRequestCount: totals.cachedRequestCount,
    refreshedRequestCount: totals.refreshedRequestCount,
    accessRequestQueue: {
...
function indexExistingClaims(existingClaims) {
  const byDomainRequest = new Map();

  for (const notice of Object.values(existingClaims.notices || {})) {
    const domain = normalizeDomain(notice.domain || "");
    const requestId = String(notice.requestId || "").trim();
    if (!domain || !requestId) continue;
    byDomainRequest.set(`${domain}::${requestId}`, notice);
  }

  return { byDomainRequest };
}

function findExistingNotice(index, domain, row) {
  const normalizedDomain = normalizeDomain(domain);
  const requestId = String(row.requestId || "").trim();
  if (!normalizedDomain || !requestId) return null;
  return index.byDomainRequest.get(`${normalizedDomain}::${requestId}`) || null;
}

function hydrateExistingNotice(domain, row, existingNotice) {
  const exactUrls = Array.isArray(existingNotice.exactUrls) ? existingNotice.exactUrls : [];
  return {
    ...existingNotice,
    domain,
    requestId: row.requestId,
    requestDate: row.date || existingNotice.requestDate || "",
    copyrightOwner: row.copyrightOwner || existingNotice.copyrightOwner || "",
    reportingOrganization: row.reportingOrganization || existingNotice.reportingOrganization || "",
    requestUrls: row.requestUrls || existingNotice.requestUrls || 0,
    percentDelisted: row.percentDelisted || existingNotice.percentDelisted || "",
    percentNotInIndex: row.percentNotInIndex || existingNotice.percentNotInIndex || "",
    googleRequestUrl: row.googleRequestUrl || existingNotice.googleRequestUrl || "",
    targetDomainUrls: Number(existingNotice.targetDomainUrls || 0),
    targetDomainUrlsNotDelisted: Number(existingNotice.targetDomainUrlsNotDelisted || 0),
    requestOutcomeCounts: existingNotice.requestOutcomeCounts || {},
    status: existingNotice.status || "access_needed",
    exactUrls,
    exactUrlCount: Number(existingNotice.exactUrlCount || exactUrls.length),
    note: existingNotice.note || "Reused from the existing claims database."
  };
}
...
function summarizeDomainResults(domainResults, notices) {
  return {
    ...
    cachedRequestCount: domainResults.reduce((sum, domain) => sum + Number(domain.cachedRequestCount || 0), 0),
    refreshedRequestCount: domainResults.reduce((sum, domain) => sum + Number(domain.refreshedRequestCount || 0), 0)
  };
}
